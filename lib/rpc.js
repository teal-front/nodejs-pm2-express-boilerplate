...
/**
 * rpc library from https://github.com/demchenkoe/node-amqp-rpc
 */
function rpc(opt) {
  if (!opt) opt = {};
  this.opt = opt;
...
rpc.prototype._connect = function (cb) {
  ...
  this.__conn.on('error', function (error) {
    console.error(error);
  });
}
...
rpc.prototype.request = function (cmd, params, cb, context, options, notDecode) {
  //console.log('cmd',cmd);
  var timer;
  var paramsString = JSON.stringify(params||{});
  this.call(methods[cmd].serverId, this.buildRequestParams(cmd, params), function (message, timestamp) {
    var encoded_payload = (notDecode ? message.data : unescape(message.data));
    ...
    clearTimeout(timer);
    cb(payload, {cmd: cmd, params: paramsString}); 
  }, context, options);

  // 超时处理
  timer = setTimeout(function(){
    cb({status:'-99', msg: '超时'}, {cmd: cmd, params: paramsString});
    cb = noop;// 已经超时则不再执行
  }, config.server.rabbitmq.timeout||90*1000);
}

rpc.prototype.requestNotDecode = function (cmd, params, cb, context, options) {
...
rpc.prototype.buildRequestParams = function (cmd, params) {
  ...
  if (method.interfaceId)
    p["command"] = (method.interfaceId?(method.interfaceId + '.'):'') + method.name;
  else
    p["command"] = (method.identity?(method.identity + '.'):'') + method.name;
  return p;
}
